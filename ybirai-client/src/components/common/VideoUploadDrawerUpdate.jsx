...
export const VideoUploadDrawerUpdate = ({
    isOpen,
    onClose,
    uploads = new Map(),
    errors = new Map(),
    ...
    const getSectionName = (sectionIndex) => {
        try {
            return syllabus[sectionIndex]?.title || `Section ${sectionIndex + 1}`;
        } catch (e) {
    ...
    const renderUploadCard = (upload, error, key) => {
        ...
        const sectionName = getSectionName(upload.sectionIndex);
                    ...
                    {error ? (
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center text-green-600">
                                ...
                                <span className="text-sm">{t('courseForm.content.upload.updateSuccess')}</span>
                            </div>
                            <Button
                                variant="ghost"
                                size="sm"
        ...
        <Sheet open={isOpen} onOpenChange={onClose}>
            <SheetContent side="right" className="w-[400px] sm:w-[540px] p-0">
                <SheetHeader className="p-6 pb-2">
                    <div className="flex items-center justify-between">
                        <div>
                            <SheetTitle>{t('courseForm.content.upload.videoUpdates')}</SheetTitle>
                            <SheetDescription>
                                {courseTitle && (
                                    <div className="text-xs text-muted-foreground mb-2">
                                        Updating: {courseTitle}
                                    </div>
                                )}
                                {hasActiveUploads
                                    ? t('courseForm.content.upload.updatingVideos', { count: groupedUploads.uploading.length })
                                    : hasErrors
                                        ? t('courseForm.content.upload.updatesFailed', { count: groupedUploads.error.length })
                                        : hasCompletedUploads
                                            ? t('courseForm.content.upload.updatesCompleted', { count: groupedUploads.completed.length })
                                            : t('courseForm.content.upload.noActiveUpdates')}
                            </SheetDescription>
                        </div>
                        ...
                        {hasCompletedUploads && (
                            <div className="mt-6">
                                <h3 className="text-sm font-medium text-gray-500 mb-4">
                                    {t('courseForm.content.upload.completedUpdates', { count: groupedUploads.completed.length })}
                                </h3>
                        ...
                        {uploads.size === 0 && (
                            <div className="text-center py-8">
                                <p className="text-muted-foreground">
                                    {t('courseForm.content.upload.noVideoUpdates')}
                                </p>
                            </div>
                        )}
                    </div>
