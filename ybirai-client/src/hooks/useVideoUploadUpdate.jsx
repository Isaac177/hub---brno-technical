...
const API_BASE_URL = 'http://localhost:8080'; // Dev URL for updates
...
export const useVideoUploadUpdate = (options = {}) => {
    const { onUploadComplete, courseId } = options;
    ...
    const uploadVideoUpdate = useCallback(async (file, sectionIndex, topicIndex, topicTitle) => {
        ...
        try {
            ...
            // Initialize upload for course update
            const formData = new FormData();
            formData.append("file", file);
            formData.append("sessionId", sessionId);
            formData.append("fileType", "VIDEO");
            formData.append("courseId", courseId); // Include course ID for updates
            formData.append("sectionIndex", sectionIndex);
            formData.append("topicIndex", topicIndex);

            uploadData = await retryOperation(
                async () => {
                    const response = await fetch(`${API_BASE_URL}/api/uploads/initiate`, {
                        method: 'POST',
                        body: formData,
                        credentials: 'include'
                    });
            ...
            console.log("Update upload initialized:", uploadData);
            ...
            for (let i = 0; i < chunks; i++) {
                ...
                const presignedData = await retryOperation(
                    async () => {
                        ...
                        const response = await fetch(`${API_BASE_URL}/api/uploads/presigned-url?${params}`, {
                            method: 'GET',
                            credentials: 'include'
                        });
                        if (!response.ok) {
                ...
                await retryOperation(
                    async () => {
                        ...
                        const response = await fetch(`${API_BASE_URL}/api/uploads/parts?${params}`, {
                            method: 'POST',
                            credentials: 'include'
                        });
                        if (!response.ok) {
            ...
            await retryOperation(
                async () => {
                    const response = await fetch(`${API_BASE_URL}/api/uploads/${uploadId}/complete`, {
                        method: 'POST',
                        credentials: 'include'
                    });
                    if (!response.ok) {
            ...
            console.log("Update upload completed, moving to final location");

            // Move to final location
            const moveResponse = await retryOperation(
                async () => {
                    const response = await fetch(`${API_BASE_URL}/api/uploads/${uploadId}/move`, {
                        method: 'POST',
                        credentials: 'include'
                    });
                    if (!response.ok) {
            ...
            console.log("Update move response:", moveResponse);
            ...
            if (uploadData?.id && uploadData?.tempKey) {
                try {
                    ...
                    await fetch(`${API_BASE_URL}/api/uploads/abort?${params}`, {
                        method: 'POST',
                        credentials: 'include'
                    });
                } catch (abortError) {
                    console.error("Failed to abort update upload:", abortError);
                }
            }

            throw error;
        }
    }, [updateUploadProgress, setUploadError, clearUploadError, onUploadComplete, courseId]);
    ...
    }, [uploads, uploadVideoUpdate, clearUploadError]);
